...
def update_params(worker_opt,values,logprobs,rewards,clc=0.1,gamma=1):
...
def run_episode(agent: StateAwareAgent, environment: Environment, worker_model, max_steps: int):
    ...
    while (done == False and j < max_steps): #C
       ...
       # print(f'Action distribution: {action_dist}')
        ...
        # print(f'Step: {j} Action: {act} Percept: {percept}')
...
def worker(t, worker_model, counter, params):
    ...
    for i in range(params['epochs']):
        ...
        initial_percept = worker_env.init(0.2, True, True)
        # worker_env.visualize()
        # print(f'Percept: {initial_percept}')
...
if __name__ == '__main__':
    ...
    MasterNode.load_state_dict(torch.load('wumpusworld_ac3 40K episodes x 12 workers mox 50000 setps with wumpus and 0.2 pits but no exit.pth'))
    ...
    path = f'wumpusworld_ac3 with pits and exit {params.epochs} episodes max {params.max_steps} steps {params.n_workers} workers.pth'
